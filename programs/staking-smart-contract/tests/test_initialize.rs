...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(
        &[staking_contract::constants::COUNTER_SEED],
        &program_id,
    )
    .0;
    ...
    let instruction = Instruction::new_with_bytes(
        ...
        staking_contract::accounts::Initialize {
            payer: payer.pubkey(),
            counter,
    ...
    let counter_state = staking_contract::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &staking_contract::instruction::Increment {}.data(),
        staking_contract::accounts::Increment {
            counter,
